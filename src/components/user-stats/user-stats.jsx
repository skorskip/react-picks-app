...
export const UserStats = () => {
    ...
    const userDetailsState = useSelector((state) => state.userDetails.status);
    
    const pickCount = parseInt(userDetails?.picks) || 0;
    const pendingCount = parseInt(userDetails?.pending_picks) || 0;
    ...
    const pickStatInfo = (userDetailsState === status.COMPLETE) && (
        <div className="picks-stat-card-group">
            ...
            <div className="pick-stat-card tiertary-color">
                <div className="large-font secondary-color">{ parseFloat(userDetails?.win_pct || 0).toFixed(2) }</div>
